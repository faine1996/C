...
/* USER CODE BEGIN PV */
static osSemaphoreId dhtSemaphoreHandle;
static osSemaphoreId printSemaphoreHandle;
static DHT11_t g_dht;
...
void StartDelayTask(void const * argument);
...
int main(void)
{
  ...
  /* USER CODE BEGIN RTOS_SEMAPHORES */
    osSemaphoreDef(dhtSemaphore);
    dhtSemaphoreHandle = osSemaphoreCreate(osSemaphore(dhtSemaphore), 1);
    osSemaphoreWait(dhtSemaphoreHandle, 0);

    osSemaphoreDef(printSemaphore);
    printSemaphoreHandle = osSemaphoreCreate(osSemaphore(printSemaphore), 1);
    osSemaphoreWait(printSemaphoreHandle, 0);
...
/* USER CODE BEGIN Header_StartDelayTask */
/**
 * @brief  Function implementing the DelayTask thread.
 * @param  argument: Not used
 * @retval None
 */
/* USER CODE END Header_StartDelayTask */
void StartDelayTask(void const * argument)
{
  /* USER CODE BEGIN 5 */
    for (;;)
    {
        ...
        osSemaphoreRelease(dhtSemaphoreHandle);
    }
  /* USER CODE END 5 */
}
...
void StartDHTTask(void const * argument)
{
  /* USER CODE BEGIN StartDHTTask */
    ...
    DHT11_Init(&g_dht, DHT_PIN_GPIO_Port, DHT_PIN_Pin);

    for (;;)
    {
        osSemaphoreWait(dhtSemaphoreHandle, osWaitForever);

        status = DHT11_Read(&g_dht, &temp, &hum);
        if (DHT11_OK == status)
        {
            g_temperature = temp;
            g_humidity = hum;
            HAL_UART_Transmit(&huart2, (uint8_t *)"DHT_OK\r\n", 8,
                              HAL_MAX_DELAY);
        }
...
void StartPrintTask(void const * argument)
{
  /* USER CODE BEGIN StartPrintTask */
    ...
    for (;;)
    {
        osSemaphoreWait(printSemaphoreHandle, osWaitForever);

        buf[0]  = 'T'; buf[1]  = 'e'; buf[2]  = 'm'; buf[3]  = 'p';
        buf[4]  = ':'; buf[5]  = ' ';
        buf[6]  = (char)('0' + (g_temperature / 10) % 10);
        buf[7]  = (char)('0' + g_temperature % 10);
        buf[8]  = ' '; buf[9]  = 'C'; buf[10] = ' '; buf[11] = ' ';
        buf[12] = 'H'; buf[13] = 'u'; buf[14] = 'm'; buf[15] = ':';
        buf[16] = ' ';
        buf[17] = (char)('0' + (g_humidity / 10) % 10);
        buf[18] = (char)('0' + g_humidity % 10);
        buf[19] = ' '; buf[20] = '%'; buf[21] = '\r'; buf[22] = '\n';
        HAL_UART_Transmit(&huart2, (uint8_t *)buf, 23, HAL_MAX_DELAY);
    }
